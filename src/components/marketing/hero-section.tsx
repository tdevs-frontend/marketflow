...
export function HeroSection() {
  return (
    <section className="hero-surface relative isolate overflow-hidden">
      <div className="mx-auto grid max-w-[1440px] items-center gap-14 px-6 py-20 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.1fr)] lg:gap-16 lg:px-12 lg:py-28 xl:gap-20 xl:px-20">
        {/* Left — message */}
        <div className="max-w-xl">
          ...
          <h1 className="mt-6 text-[2.75rem] font-bold leading-[1.06] tracking-tight text-balance sm:text-6xl xl:text-[4.25rem]">
            Turn Every Conversation Into <span className="text-primary-dark">Growth</span>
          </h1>

          <p className="mt-6 max-w-lg text-lg leading-relaxed text-text-secondary text-pretty">
            Manage leads, automate WhatsApp conversations, launch campaigns, and turn customer
            interactions into measurable business growth.
          </p>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="flex items-center gap-1">
                  {[0, 1, 2, 3, 4].map((star) => (
                    <Star key={star} className="h-3.5 w-3.5 fill-warning text-warning" aria-hidden />
                  ))}
                  <span className="ml-1 text-xs font-semibold text-text-primary">4.9/5</span>
                </div>
                <p className="mt-0.5 text-xs text-text-muted">
                  Trusted by{" "}
                  <span className="font-semibold text-text-secondary">2,400+ merchants</span> across
                  18 countries
                </p>
              </div>
            </div>
