...
/**
 * Step 4 — merge tags, and what happens when the data behind one is missing.
 *
 * The step exists for the fallback column. A campaign that opens "Hi {{first_name}},"
 ...
 * tag in the draft is listed here whether or not it has a value, and the ones
 * without a fallback are called out rather than sorted to the bottom.
 */

export function PersonaliseStep(props: StepProps) {
  const { draft, set, derived, goTo } = props;
    ...
    <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_22rem]">
        ...
        <StepSection
          ...
          hint={
            derived.isSocial
              ...
              : "Click one to copy it, then paste it into the message on the Content step."
          }
        >
          <div className="space-y-3">
            ...
            {derived.isSocial ? (
              <p className="flex items-start gap-2 rounded-panel bg-surface-secondary px-3.5 py-2.5 text-sm font-medium text-text-secondary">
                ...
                person. If direct messaging arrives on a platform, those
                variables become available for that flow.
              </p>
            ) : null}
          </div>
        </StepSection>
          ...
          >
            ...
            {derived.missingFallbacks.length > 0 ? (
              <WarningNote>
                ...
                fallback. Any contact missing that field receives the tag
                literally — set a fallback here, or write one inline as{" "}
      ...
      <div className="lg:sticky lg:top-4 lg:self-start">
        ...
        <p className="mt-2 text-sm text-text-muted">
          Rendered for Sarah Ahmed at Bright Retail. Fields she has no value for
          fall back to what you set here.
        </p>
      </div>
    </div>
  );
