...
function PreviewBlock({ block }: { block: EmailBlock }) {
  switch (block.type) {
    ...
    case "text":
      return (
        <p className="px-6 pt-3 text-sm leading-relaxed text-text-secondary">
          {block.content}
        </p>
      );
      ...
      return (
        <div className="px-6 pt-4">
          <div className="grid aspect-[3/1] place-items-center rounded-panel bg-email-soft">
            <p className="text-sm font-medium text-email">
              {block.content}
    ...
    case "button":
      return (
        <div className="px-6 pt-5">
          <span className="inline-flex h-10 items-center rounded-btn bg-email px-5 text-sm font-semibold text-white">
            {block.content}
          </span>
        </div>
      );
            ...
            {PALETTE.map((type) => {
              ...
              return (
                <button
                  ...
                  type="button"
                  onClick={() => add(type)}
                  className="flex items-center gap-1.5 rounded-btn border border-border px-2 py-1.5 text-sm font-medium text-text-secondary transition-colors hover:border-email hover:bg-email-soft hover:text-email-dark focus-visible:shadow-focus focus-visible:outline-none"
                >
                  <Icon className="size-3.5 shrink-0" aria-hidden />
