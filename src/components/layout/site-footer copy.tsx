...
/**
 * How a link behaves on this panel: two pixels sideways, and up to full white.
 *
 * The transition names `translate`, not `transform` — Tailwind v4 writes its
 * translate utilities to the former, so `transition-transform` here would
 * animate nothing at all.
 */
const LINK = [
  "text-white/66 transition-[color,translate] duration-200 ease-out",
...
/**
 * The panel's ground: four decorative layers, clipped by the panel's own
 ...
 *    atmosphere: a 60px blur on a shape half the panel wide has no edge
 *    anywhere, so the light never gives away a boundary. The top-right one is
 *    the reference's signature, and its centre sits *above* the panel — which
 ...
 *    makes the reference's bright top survivable: `rgba(255,255,255,.66)`
 *    links measure 3.3:1 against that top-right light and 6:1 against the same
...
/**
 * The bottom of the product: one rounded gradient panel, inset from the
 * viewport on all four sides so the page above reads as a page the card sits
 * on. That inset is what lets the ground be this saturated without the bottom
 * of the screen turning into a slab.
 *
...
export function SiteFooter() {
  return (
    <footer className="px-6 pb-6 sm:px-8 lg:pb-8">
      <div className="relative isolate mx-auto max-w-360 overflow-hidden rounded-[22px] border border-white/10 shadow-[inset_0_1px_0_rgba(255,255,255,0.10),0_30px_80px_rgba(15,23,42,0.12)] sm:rounded-[30px] lg:rounded-[34px]">
        <FooterGround />

        <div className="px-6 pt-12 pb-6 sm:px-10 sm:pt-14 lg:px-18 lg:pt-16 lg:pb-8 xl:px-21">
          {/*
          ...
          <div className="grid gap-x-8 gap-y-12 sm:grid-cols-2 lg:grid-cols-[minmax(0,1.4fr)_repeat(3,minmax(0,1fr))] lg:gap-x-10">
            <div className="sm:col-span-2 lg:col-span-1">
               ...
               * on this panel where the identity ramp appears as a fill, and
            ...
            {COLUMNS.map((column) => (
              ...
              >
                <h2
                  ...
                  className="text-base font-semibold tracking-tight text-white capitalize"
                >
                  {column.title}
                </h2>
                <ul className="mt-4 space-y-3">
                  {column.items.map((item) => (
                    <li key={item.href}>
                      <Link
                        href={item.href}
                        className={`inline-block text-[15px] ${LINK}`}
                      >
                        {item.title}
                      </Link>
                    </li>
